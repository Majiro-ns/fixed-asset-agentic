...
<!-- TODO: 殿がYouTube/Vimeoにアップロード後、以下を置き換える -->
<!-- @[youtube](動画ID) -->

:::message
デモ動画（3分）--- 撮影後にURLを埋め込み
:::
...
<!-- TODO: スクリーンショット: GUIDANCE画面（2択ボタン） -->

### なぜ「止まる」のか

Gemini 3 Proに税法知識を注入し、**書類全体の文脈**から判定している。以下の条件で自律的に停止する：
...
GUIDANCEで停止した明細には、AIによる参考判定（`ai_hint`）が付与される場合がある。判定はしない（GUIDANCE維持）が、AIの見解は参考として提示する設計だ。Gemini APIによる推定を第一候補とし、タイムアウト時はキーワードヒューリスティックにフォールバックする。UIでは「AI参考判定: 費用寄り (信度60%)」のように青色バッジで表示され、ユーザーが判断する際の補助情報として機能する。
...
<!-- TODO: スクリーンショット: 2択ボタン + 2段階質問 -->

### ステップ4：再実行

ユーザーの回答を踏まえてAIが再判定する。

### ステップ5：差分を見せる

Before → After で何が変わったかを明示する。

<!-- TODO: スクリーンショット: Before/After差分表示 -->

> **この差分は監査時の説明資料として利用できます。**

判定が変わった理由が、証跡として自動記録される。税務調査で「なぜこの処理にしたのですか？」と問われた時、この記録が回答になる。

---

## 似た事例の提示（セレンディピティ）

GUIDANCEで止まった時、AIが**似た実務事例**を1つ提示する。

> **似た事例：** 空調設備の更新工事で撤去費を含むケース
> **結果：** 撤去費を除外し、資本的支出80万円＋修繕費40万円に分離
> **教訓：** 「一式」の中に撤去費が含まれる場合、分離計上が有利なことがある

ユーザーが自分では気づかなかった視点を、AIが提供する。**判定だけでなく、判断の質を上げる支援**だ。
...
Gemini 3 Proのプロンプトに、以下の税法知識を体系的に組み込んでいる。
...
- これにより、明確な経費（「撤去」等）が金額閾値で誤って資産扱いされることを防ぐ
...
**これらの閾値に該当する場合、キーワード判定に関わらずGUIDANCEで安全側に倒す。**
...
CAPITAL_LIKE判定時に、国税庁「減価償却資産の耐用年数等に関する省令」に基づく資産区分と法定耐用年数を自動表示。
...
月末20枚の見積書処理: 5時間 → 1時間40分（3時間20分の削減）
